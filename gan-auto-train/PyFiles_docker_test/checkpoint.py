...
def save_checkpoint(generator, discriminator, epoch, output_dir):
    ...
    generator.save_weights(os.path.join(gen_path, 'weights.h5'))
    discriminator.save_weights(os.path.join(disc_path, 'weights.h5'))

    print(f"[✓] Saved checkpoint at epoch {epoch} to {output_dir}")
