...
class GraphGenerator_test(tf.keras.Model):
  ...
  def call(self, z):
    ...
    adj = tf.clip_by_value(adj, 0, 4)
    adj = (adj + tf.transpose(adj, perm=[0, 2, 1])) / 2
    print("Discrete adjacency matrix (bond types):\n", adj.numpy())
    ...
    node_features = tf.argmax(node_features, axis = -1)
    ...
    for epoch in range(epochs):
      ...
      for z in data:
        with tf.GradientTape() as tape:
          ...
          fake_output = GraphDiscriminator(gen_adj, gen_z, conditions)
...
class GraphGenerator_test(tf.keras.Model):
  def __init__(self, num_nodes, node_features, latent_dim):
     ...
     self.mlp = models.Sequential([
         layers.Dense(128, input_shape = (latent_dim,), activation="leaky_relu"),
         layers.Dense(256, activation = "leaky_relu"),
         layers.Dense((num_nodes * num_nodes * 5) + (num_nodes * node_features), activation = "softmax")
     ])
  ...
  def call(self, z):
    ...
    adj = tf.reshape(adj_flat, (batch_size, self.num_nodes, self.num_nodes, 5))
    adj = tf.nn.softmax(adj, axis=-1)
    adj = tf.argmax(adj, axis=-1)
    adj = tf.cast(adj, tf.float32)
    adj = (adj + tf.transpose(adj, perm=[0, 2, 1])) / 2

    valid_atom_types = tf.constant([1, 6, 7, 8, 9], dtype = tf.int64)

    node_features = tf.reshape(node_feature_flat, (batch_size, self.num_nodes, -1))
    node_features = tf.nn.softmax(node_features, axis = -1)
    node_features = tf.argmax(node_features, axis = -1)
    node_features = tf.gather(valid_atom_types, node_features)
...
class GraphDiscriminator(tf.keras.Model):
    ...
    def call(self, adj, node_features, condition):
        batch_size = tf.shape(adj)[0]

        adj_flat = tf.reshape(adj, (batch_size, -1))
        node_features_flat = tf.reshape(node_features, (batch_size, -1))

        combined = tf.concat([adj_flat, node_features_flat, condition], axis = 1)
...
class GraphDiscriminator_test(tf.keras.Model):
    ...
    def call(self, adj, node_features):
        ...
        print(f"adj_flat shape: {adj_flat.shape}")
        print(f"node_features_flat shape: {node_features_flat.shape}")
